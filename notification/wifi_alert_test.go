...
func TestDescribeWiFiAlertDetails(t *testing.T) {
	cases := []struct {
		...
		{
			name:   "disconnected",
			detail: FaultDetail{ProbeKind: "host", MetricKind: "wifi.up", Comparator: "lt", Threshold: 1, Value: 0, Target: "wlan0", AgentHost: "node-wifi"},
			zh:     []string{"Wi-Fi 网卡 wlan0", "断开连接", "来自 node-wifi"},
			en:     []string{"Wi-Fi adapter wlan0", "disconnected", "on node-wifi"},
		},
		{
			name:   "low signal",
			detail: FaultDetail{ProbeKind: "host", MetricKind: "wifi.signal_dbm", Comparator: "lt", Threshold: -70, Value: -75, Target: "en0"},
			zh:     []string{"Wi-Fi 网卡 en0", "信号强度 -75 dBm", "阈值 < -70 dBm"},
			en:     []string{"Wi-Fi adapter en0", "signal strength is -75 dBm", "threshold < -70 dBm"},
		},
		{
			name:   "low quality",
			detail: FaultDetail{ProbeKind: "host", MetricKind: "wifi.quality_pct", Comparator: "lt", Threshold: 60, Value: 45, Target: "Wi-Fi"},
