...
// TestFreezeReasonDetailFromOverlay verifies that when the reason is read from the
// accepted-batch overlay, the raw cause carried on the error_class sample's
// "detail" label is frozen alongside the code — and that the store-path fallback
// (the latest cache carries no labels) freezes '' (the designed degradation).
func TestFreezeReasonDetailFromOverlay(t *testing.T) {
	const detail = "write udp 0.0.0.0->1.1.1.1: sendto: network is unreachable"
	cases := []struct {
		name       string
		useOverlay bool
		wantDetail string
	}{
		{"overlay freezes the label detail", true, detail},
		{"store path degrades to empty detail", false, ""},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			db, ctx, cfg, groupID := openRulesTest(t)
			...
			if tc.useOverlay {
				...
				overlay := BuildOverlay([]telemetry.Metric{
					{TS: now, Kind: telemetry.ICMPLoss, Target: "1.1.1.1", Value: 100, MonitorID: "cf"},
					{TS: now, Kind: telemetry.ICMPErrorClass, Target: "1.1.1.1", MonitorID: "cf",
						Value:  float64(telemetry.ProbeReasonUnreachable),
				...
				if _, err := svc.EvaluateAgentTx(ctx, tx, "agent_a", "site_default", overlay); err != nil {
					_ = tx.Rollback()
					t.Fatalf("evaluate in tx: %v", err)
				}
				...
				seedMonitorSampleAt(t, db, "agent_a", "cf", "probe.icmp.loss_pct", "1.1.1.1", 100, ts)
				seedMonitorSampleAt(t, db, "agent_a", "cf", string(telemetry.ICMPErrorClass), "1.1.1.1",
					float64(telemetry.ProbeReasonUnreachable), ts)
				if err := svc.EvaluateAgent(ctx, "agent_a", "site_default"); err != nil {
					t.Fatalf("evaluate: %v", err)
				}
			}
			...
			var got string
			if err := db.QueryRowContext(ctx, `
				SELECT reason_code, reason_detail FROM alert_evidence
				WHERE metric_kind='probe.icmp.loss_pct' ORDER BY observed_at DESC LIMIT 1`).Scan(&reason, &got); err != nil {
				t.Fatalf("read evidence: %v", err)
			}
			if reason != telemetry.ProbeReasonUnreachable {
				t.Fatalf("frozen reason_code = %d, want %d", reason, telemetry.ProbeReasonUnreachable)
			}
			if got != tc.wantDetail {
				t.Fatalf("frozen reason_detail = %q, want %q", got, tc.wantDetail)
			}
		})
	}
}
